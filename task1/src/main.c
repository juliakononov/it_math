...
double block_processing(int bi, int bj, meta *meta)
{
    ...
    for (int i = 1 + bi * meta->block_size; i <= MIN((bi + 1) * meta->block_size, meta->size - 2); i++)
    {
        for (int j = 1 + bj * meta->block_size; j <= MIN((bj + 1) * meta->block_size, meta->size - 2); j++)
        {
            temp = meta->u[i][j];
            meta->u[i][j] = 0.25 * (meta->u[i - 1][j] + meta->u[i + 1][j] +
                                    meta->u[i][j - 1] + meta->u[i][j + 1] -
                                    meta->h * meta->h * meta->f[i][j]);
...
double d_sinx_p_cosy3(double x, double y) { return -sin(x) - 3 * (1 - 3 * pow(sin(y), 2)) * cos(y); }

int main()
{
    FILE *fpt;
    fpt = fopen("results.csv", "w");
    fprintf(fpt, "Thread number, Size, Block size, Epsilon, Iteration number, Time, Boost, Maximal error, Average error\n");
...

        {
            for (bs = 0; bs < 4; bs++)
            {
                for (e = 0; e < 2; e++)
                {
                    ...
                    if (nt == 0)
                    {
                        fprintf(fpt, "1, %d, %d, %f, %ld, %7.4f, -, %7.2f, %7.2f\n", size[sz], block_size[bs], eps[e], mt->iter, time, d, average_err);
                        seq[sz][bs][e] = time;
                    }
                    else
                    {
                        double boost = seq[sz][bs][e] / time;
                        fprintf(fpt, "%d, %d, %d, %f, %ld, %7.4f, %7.2f, %7.2f, %7.2f\n", num_threads[nt], size[sz], block_size[bs], eps[e], mt->iter, time, boost, d, average_err);
                    }
                    free_meta(mt);
                }
            }
